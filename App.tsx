...
export default function App() {
  ...
  const [status, setStatus] = useState('Initializing Cloud Connection... ☁️');
  ...
  // Initial Data Fetch from Google Sheet
  useEffect(() => {
    const loadSheetData = async () => {
      setIsDataLoading(true);
      try {
        const data = await fetchSheetData();
        if (data && (data.CHARACTER.length > 0 || data.SCENE.length > 0)) {
          setAppData(data);
          setStatus('Cloud Data Loaded Successfully! ☁️✨');
        } else {
           setStatus('Using Offline Mode (Cloud Load Failed) ⚠️');
           // Fallback is already initial state
        }
      } catch (err) {
        console.error("Failed to load sheet data", err);
        setStatus('Offline Mode - Ready to Create! 🎨');
      } finally {
  ...
  const selectionCount = useMemo(() => {
    return Object.values(selections).reduce((acc, section) => 
  ...
  const generatePrompt = useCallback(() => {
    ...
    setTimeout(() => {
      const parts: string[] = [];
      Object.values(selections.character).filter(Boolean).forEach(v => parts.push(v));
      Object.values(selections.scene).filter(Boolean).forEach(v => parts.push(v));
      Object.values(selections.camera).filter(Boolean).forEach(v => parts.push(v));
  ...
  const currentCategories = appData[activeSection] || [];
  ...
  if (isDataLoading) {
      return (
        <div className="bg-[color:var(--bg-primary)] text-[color:var(--text-primary)] min-h-screen flex flex-col items-center justify-center transition-colors duration-500">
             <div className="w-16 h-16 border-4 border-[color:var(--accent-2)] border-t-[color:var(--accent-1)] rounded-full animate-spin mb-4"></div>
             <h2 className="text-xl font-bold animate-pulse">Connecting to Cloud Knowledge Base... ☁️</h2>
        </div>
      );
  }
    ...
    <div className="bg-[color:var(--bg-primary)] text-[color:var(--text-primary)] min-h-screen p-2 sm:p-5 transition-colors duration-500">
                    ...
                    {currentCategories.map(cat => (
                        <div key={cat.name} className="p-3 bg-[color:var(--bg-secondary)] border-2 border-[color:var(--accent-2)] rounded-lg grid grid-cols-[1fr,auto] gap-2 items-center">
                            <label className="font-semibold text-sm text-[color:var(--accent-4)] flex items-center gap-2">
                                {cat.name}
                                <span className="cursor-help" onMouseEnter={(e) => showTooltip(e, `Source: ${cat.src.sheet} • Col ${cat.src.col}`)} onMouseLeave={hideTooltip}>ℹ️</span>
      ...
      <Modal isOpen={isMultiFormatModalOpen} onClose={() => setMultiFormatModalOpen(false)} title="📜 Multi-Format Prompts">
        <div className="space-y-4 max-h-[60vh] overflow-y-auto p-1">
          {/* FIX: Use Object.keys with a type assertion to ensure type safety when iterating over multiFormatPrompts.
              This resolves the TypeScript error where `value` was inferred as `unknown` from `Object.entries`. */}
          {multiFormatPrompts && (Object.keys(multiFormatPrompts) as Array<keyof MultiFormatPrompts>).map((key) => {
            const value = multiFormatPrompts[key];
            return (
